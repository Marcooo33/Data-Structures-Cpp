...
namespace lasd {
  ...
  protected:
    ...
    struct Node {
      ...
      inline Node(Data&& dat) : Element(std::move(dat)) {};

      /* ********************************************************************** */

      // Copy constructor
      inline Node(const Node& node) noexcept : Element(node.Element) {};
  ...
  public:
    ...
    List(const TraversableContainer<Data>& con); // A list obtained from a TraversableContainer
    List(MappableContainer<Data>&& con); // A list obtained from a MappableContainer
